...
<xsl:variable name="path" select="translate($package, '.', '/')"/>
		...
		<xslt 	in="res/service/{$serviceTypeVersion}.xml"
			out="${{generation.dir}}/src/{$path}/model/{$classname}Model.java"
			style="scdp2modelitf.xsl" 
			classpath="."
		>
			<param name="date" expression="${{date}}"/>
			<param name="author" expression="${{author}}"/>
		...
		<xslt 	in="res/service/{$serviceTypeVersion}.xml"
			out="${{generation.dir}}/src/{$path}/impl/{$classname}ModelImpl.java"
			style="scdp2modelimpl.xsl" 
			classpath="."
		>
			<param name="date" expression="${{date}}"/>
			<param name="author" expression="${{author}}"/>
		...
		<echo file="${{generation.dir}}/res/upnpservicemap.txt" append="true"
				message="{serviceType}={$package}.model.{$classname}Model;{$package}.controlpoint.{$classname}ProxyImpl"/>
		<echo file="${{generation.dir}}/res/upnpservicemap.txt" append="true">&#13;&#10;</echo>				
